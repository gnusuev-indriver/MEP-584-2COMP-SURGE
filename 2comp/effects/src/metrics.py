...
METRIC_LIST = [
    ...
    # nusuev
    ["orders_by_recprice_share", "orders_recprice_count", "orders_count"]
]
...
def get_switchback_results_new(df, alpha, metric_list=METRIC_LIST, groups={"control": "Control", "treatment": "A"}):
    res_list = []
    for i in metric_list:
        pipeline = RatioMetricHypothesisTestingPipeline(df, i[0], i[1], i[2], groups)
        try:
            ...
            print(f"ValueError: {VE} for metric {i}")
        except KeyError as KE:
            print(f"KeyError: {KE} for metric {i}")
        except Warning as W:
            print(f"Warning: {W} for metric {i}")
        except Exception as e:
            print(f"Unexpected Error: {e} for metric {i}")
        finally:
            # Log metric causing potential issues
            print(f"Completed processing metric {i}")
        res_list.append(getattr(pipeline, "result", None))  # Append result if it exists
    df_res = pd.DataFrame(filter(None, res_list))  # Filter out None results
    df_res["is_significant"] = df_res["pvalue"] < alpha
    return df_res
...
# nusuev
def metric_orders_by_recprice_neighborhood_10_count(df, group_cols):
    return (df[df.price_start_usd / df.price_highrate_usd >= 0.9]
            .groupby(group_cols)
            .agg(orders_recprice_count=('order_uuid', 'nunique'))
            .reset_index())
...
def calculate_metrics(df_recprice, df_order, df_full, group_cols):
    dfm = (metric_calcprices_count(df_recprice, group_cols)
           ...
           .merge(metric_orders_by_recprice_neighborhood_10_count(df_order, group_cols), on=group_cols, how='left')
